...
function CommandPaletteDialog({
  ...
  const filteredCommands = useMemo(
    () => filterCommands(commands.map((command) => ({ ...command, title: t(command.title), subtitle: command.subtitle ? t(command.subtitle) : undefined })), query),
    [commands, query, t]
  );
      ...
      >
        ...
        <div className="flex items-center px-4 py-3.5 border-b border-neutral-100 dark:border-neutral-800 gap-3">
          ...
          <input
            ...
            placeholder={t("Type a command or search actions...")}
        ...
        <div className="max-h-80 overflow-y-auto p-2">
          {filteredCommands.length === 0 ? (
            <div className="p-8 text-center text-sm text-neutral-400">
              {t("No matching commands found for “{query}”", { query })}
            </div>
          ) : (
        ...
        <div className="px-4 py-2.5 bg-neutral-50 dark:bg-neutral-950/60 border-t border-neutral-100 dark:border-neutral-800 flex items-center justify-between text-xs text-neutral-400">
          ...
          <span className="text-[11px]">{t("Mailflare Actions")}</span>
        </div>
      </div>
    </div>
  );
